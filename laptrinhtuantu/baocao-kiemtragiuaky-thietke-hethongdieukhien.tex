...
\title{\textbf{Báo cáo kiểm tra giữa kỳ \vspace{.4cm} \\ Chủ đề -- Kỹ thuật lập trình tuần tự\vspace{.4cm}\\Môn học Thiết kế hệ thống điều khiển}}
...
\subsection*{Yêu cầu}
	\begin{enumerate}
		...
		\item Sử dụng \emph{phần mềm Festo Fluidsim} \emph{mô phỏng quá trình} cho trên \emph{hình \ref{Fig:debai-baocao}} để kiểm chứng lại kết quả trong câu \ref{ex:sodologic}.
	\end{enumerate}
	
\subsection*{Danh sách sinh viên nhóm 4}
	\begin{center}
		\vspace{.5cm}
		\begin{tabular}{|c|p{3.5cm}|c|}\hline
			\textbf{STT} & \centering{\textbf{Họ và tên SV}} & \textbf{MSSV} \\ \hline
			1 & Nguyễn Văn Đình & 1350353 \\ \hline
			2 & Thi Minh Nhựt & 1350366 \\ \hline
			3 & Phạm Thanh Quý & 1350222 \\ \hline
			4 & Hồ Minh Thành & 1350444 \\ \hline
			5 & Liên Thái Trường & 1350358 \\ \hline
			6 & Lư Anh Tuấn & 1350240 \\ \hline
		\end{tabular}
	\end{center}
\section{Nội dung báo cáo}
\subsection*{Bài tập 4 -- Nhóm 4}
Cho sơ đồ quá trình như hình \ref{Fig:quatrinh-tuantu-baitap4}:
			\begin{figure}[!h]
				\begin{center}
					\begin{tikzpicture}[>=triangle 45]
						\draw (5,0) node {\small{$Start~A+D+D-G+G-A-A+B+A-B-\left\{\begin{array}{c}D+E+E-D-\\ \\G+G-H+H-\end{array}\right\}E+B+B-H+E-H-$}};
			...
			\begin{itemize}
				...
				Thực hiện chia nhóm và đặt tên cho các nhóm được sơ đồ như hình \ref{Fig:quatrinh-tuantu-baitap4-gomnhom}.
					\begin{figure}[!h]
						\begin{center}
							\begin{tikzpicture}[>=triangle 45]
								\draw (5,0) node {\small{$Start~A+D+D-G+G-A-A+B+A-B-\left\{\begin{array}{c}D+E+E-D-\\ \\G+G-H+H-\end{array}\right\}E+B+B-H+E-H-$}};
								\draw (1.9,-0.3) -- (1.9,-1) -- (-2.7,-1); \draw (-0.4,-1) node[above]{$2 \textrm{ lần }$};
						
								\draw (13.8,-0.3) -- (13.8,-2) -- (-2.7,-2); \draw[->](-2.7,-2) -- (-2.7,-0.3); \draw (12.55,-2) node[above]{$\overline{Stop}$};
				...
				\item \emph{Bước 4: Viết sơ đồ cấp điện.} Khi viết sơ đồ cấp điện chúng ta không cần quan tâm đến vòng lặp của qá trình, sơ đồ cấp điện được biểu diễn trên hình \ref{Fig:quatrinh-tuantu-baitap4-sodocapdien}.
					\begin{figure}[!h]
						\vspace{-.5cm}
						\begin{center}
							\begin{tikzpicture}[>=triangle 45]
